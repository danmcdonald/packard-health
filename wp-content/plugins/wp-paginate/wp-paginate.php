...
if (!class_exists('WPPaginate')) {
	class WPPaginate {
		...
		function paginate($args = false) {
			...
			if ($pages > 1) {
				$output .= sprintf('<ul class="wp-paginate%s pagination">', ($this->type === 'posts') ? '' : ' wp-paginate-comments');
				...
				if ($page > 1 && !empty($previouspage)) {
					$output .= sprintf('<li class="previous"><a href="%s" class="prev">%s</a></li>', $prevlink, stripslashes($previouspage));
				}
				...
				if ($page < $pages && !empty($nextpage)) {
					$output .= sprintf('<li class="next"><a href="%s" class="next">%s</a></li>', $nextlink, stripslashes($nextpage));
				}
				$output .= "</ul>";
			}
		...
		function paginate_loop($start, $max, $page = 0) {
			...
			for ($i = $start; $i <= $max; $i++) {
				...
				$output .= ($page == intval($i))
					? "<li class=\"active\"><span class='page current'>$i</span></li>"
